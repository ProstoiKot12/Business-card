...
Business Card Bot — Telegram-бот и дублирующий Web App для демонстрации портфолио разработчика. Клиенты листают кейсы, считают стоимость проекта через интерактивный калькулятор, читают FAQ и связываются через удобные каналы. Владелец управляет контентом через встроенную админ-панель прямо в Telegram.

## 🧭 КАРТА ВОЗМОЖНОСТЕЙ

- **👤 Посетители:** Портфолио-карусель, калькулятор стоимости, FAQ, контакты и ссылки.
- **🌐 Web App:** Полноценный веб-интерфейс внутри Telegram — те же сценарии с визуальным оформлением.
